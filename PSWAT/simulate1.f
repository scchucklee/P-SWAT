...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      allocate(altotbegin(nprocs))
      allocate(altotdur(nprocs))
      allocate(totlasthruper(nprocs))
      allocate(lastsubarr(nprocs))
      allocate(subbeginarr(nprocs))
      allocate(subamountarr(nprocs))
      end if
      allocation = 1
...
!      write (*,*) 'hrusper in core',myid,'=',hrusper         
